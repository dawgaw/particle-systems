	...
	constructor(private count: number, private spacing: number, k: number = 0.01) {
		...
		for (let i = 0; i < this.count; i++) {
			for (let j = 0; j < this.count; j++) {
				...
				if (j) {
					this.springs.push(new Spring(k, spacing, this.particles[i * this.count + j - 1], this.particles[i * this.count + j]))
				}
			}
			if (i) {
				for (let j = 0; j < this.count; j++) {
					this.springs.push(new Spring(k, spacing, this.particles[(i - 1) * this.count + j], this.particles[i * this.count + j]))
				}
			}
		}
	}
